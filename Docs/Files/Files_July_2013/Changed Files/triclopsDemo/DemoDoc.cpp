...
//03-30-2012 Yixin Geng
ON_COMMAND(ID_EXPORT_HIGH, OnFileExportHighResolution)
...
void CDemoDoc::OnFileExportHighResolution() 
{

	//Dipti Killedar
	//To check for 3d window first before exporting.

	CMainFrame m_frame;
	if (m_frame.window_flag == 0)
	{
		AfxMessageBox("Take a picture before export function.");
        return;
	}

	CPGRStereoDoc::OnFileExportHighResolution(m_dialogStereo.base_line,m_dialogStereo.smsurface);
}
