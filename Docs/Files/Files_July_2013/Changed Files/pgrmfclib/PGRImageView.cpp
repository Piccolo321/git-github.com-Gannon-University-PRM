...
void CPGRImageView::OnDraw( CDC* pDC )
{
   ...
   /* Draw Rectangle */
	...
	//Yixin Geng
	//Draw Center line
	/*centerLineLeftEnd.x = m_ptStart.x;
	centerLineLeftEnd.y = (m_ptStart.y + m_ptEnd.y)/2;
	centerLineRightEnd.x = m_ptEnd.x;
	centerLineRightEnd.y = (m_ptStart.y + m_ptEnd.y)/2;

	pDC->MoveTo(centerLineLeftEnd);
	pDC->LineTo(centerLineRightEnd);*/
...
void CPGRImageView::OnLButtonUp( UINT nFlags, CPoint point ) 
{
   ...
   CView::OnLButtonUp( nFlags, point );
	...
	//Yixin Geng
   /*
     * If the Left Upper Corner Point of Rectangle is selected
     * then it moves the Rectangle. 
     * If lower right is selected, rectangle resizes
     */
   if(flag == 1)
   {	  
	    m_ptStart = point;
