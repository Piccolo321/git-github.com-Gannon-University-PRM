...
{	
	...
	// To check whether pointlist is available or not.
	if(pointList->GetSize() < 1)
	{
		AfxMessageBox("Take a picture before export function. Go to 'Window' menu and select 'Take Picture'. And then go to 'Export' menu.");
		return;
	}
	else
	{
        ...
        if(fileDialog.DoModal() == IDOK)
        {
			if(!resolutionPoints_seatPart->saveToMillFormat(fileDialog.GetPathName()+".dat",97,81))
			{
				AfxMessageBox("Failed to save point cloud file ");
			}
	    }
