...
CPGRImageView::CPGRImageView()
{
	...
	m_greenBrush.CreateSolidBrush(PS_NULL);

	//Dipti Killedar
	//Fang Yuan
	// red square's co-ordinate
	m_ptStart.x = 46;
    m_ptStart.y = 52;
    m_ptEnd.x = 300;
    m_ptEnd.y = 282;
    ...
    //flag indicates if the red square has been moved
	flag = 0;

	maxXsquare_row = m_ptEnd.x - m_ptStart.x;
	maxYsquare_col = m_ptEnd.y - m_ptStart.y;
}
...
BOOL CPGRImageView::PreCreateWindow( CREATESTRUCT& cs )
{
...
void CPGRImageView::OnDraw( CDC* pDC )
{
   ...
   /* Draw Rectangle */
    CBrush  *pBrushSv = pDC->SelectObject(&m_greenBrush);
    CPen    *pPenSv = pDC->SelectObject(&m_redPen);
...
void CPGRImageView::OnMouseMove( UINT nFlags, CPoint point ) 
{
...
void CPGRImageView::OnLButtonUp( UINT nFlags, CPoint point ) 
{
   ...
   CView::OnLButtonUp( nFlags, point );
	 ...
	 //Yixin Geng
   /* If the Left Upper Corner Point of Rectangle is selected then it moves 
	   the Rectangle. If lower right is selected, rectangle resizes. */

   // Assign new values to the redframe's co-ordinate.
   if(flag == 1)
   {	  
	    m_ptStart = point;
	   	m_ptEnd.x = m_ptStart.x + maxXsquare_row;
	  	m_ptEnd.y = m_ptStart.y + maxYsquare_col;
   }

   // Set ROI with new values.
   pDoc->setROI(m_ptStart,m_ptEnd);
   
	 double midx1 = (m_ptEnd.x + m_ptStart.x) / 2;
   double midy1 = ((m_ptEnd.y + m_ptStart.y)) / 2;

   pDoc->setCenterPixelXY(midx1,midy1);

}

void CPGRImageView::customDraw( CDC* /* pDC */ )
{
...
void CPGRImageView::OnInitialUpdate() 
{
   ...
   // Set ROI with initial co-ordinates of redframe.
   pDoc->setROI(m_ptStart,m_ptEnd);

   // Calculate center point of redframe and set it.
   double  midx = (m_ptEnd.x + m_ptStart.x) / 2;
   double  midy = ((m_ptEnd.y + m_ptStart.y)) / 2;
   pDoc->setCenterPixelXY(midx,midy);

   resizeToResolution(pDoc->getResolution());
...
void CPGRImageView::OnUpdate( CView*, LPARAM, CObject* ) 
{
...
void CPGRImageView::resizeToResolution( StereoImageResolution resolution )
{
   int	 iCols = 0;
   int	 iRows = 0;
...
void CPGRImageView::resizeToDimensions( int iRows, int iCols )
{
   CWnd* pframe = GetParentFrame(); 
...
void CPGRImageView::changeDisplay()
{
   CPGRStereoDoc* pDoc = (CPGRStereoDoc*)GetDocument();
   ASSERT_VALID( pDoc );
...
void CPGRImageView::OnSelchangeIdcComboCamera() 
{
   CPGRMainFrame* pFrame = (CPGRMainFrame*)AfxGetMainWnd();
   ASSERT_VALID( pFrame );
...
void CPGRImageView::OnSelchangeIdcComboImage() 
{
   CPGRMainFrame* pFrame = (CPGRMainFrame*)AfxGetMainWnd();
   ASSERT_VALID( pFrame );
   pFrame->m_dialogBarStereo.AssertValid();
...
void CPGRImageView::OnActivateView( BOOL bActivate, CView* pActivateView, CView* pDeactiveView ) 
{
   CPGRStereoDoc*  pDoc = GetDocument();
   ASSERT_VALID( pDoc );
...
/*
 =======================================================================================================================
    On Left ButtonDown Function
 =======================================================================================================================
 */
void CPGRImageView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    float   fMapPointx = Rect.right - Rect.left;
    float   fMapPointy = Rect.bottom - Rect.top;
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

    switch(pDoc->getResolution())
    {
    case STEREO_160x120:     fMapPointx = 160 / fMapPointx; fMapPointy = 120 / fMapPointy; break;
    case STEREO_320x240:     fMapPointx = 320 / fMapPointx; fMapPointy = 240 / fMapPointy; break;
    case STEREO_640x480:     fMapPointx = 640 / fMapPointx; fMapPointy = 480 / fMapPointy; break;
    case STEREO_1024x768:    fMapPointx = 1024 / fMapPointx; fMapPointy = 768 / fMapPointy; break;
    case STEREO_1280x960:    fMapPointx = 1280 / fMapPointx; fMapPointy = 960 / fMapPointy; break;
    default:                    fMapPointx = 320 / fMapPointx; fMapPointy = 240 / fMapPointy;
    }

    long    pointx = point.x * fMapPointx;
    long    pointy = point.y * fMapPointy;
    ...
    case IMAGE_DISPARITY:
        {
            ...
            if(pDoc->getPointData(pointy,pointx,&dDisparity,&dX,&dY,&dZ))
            {
                sprintf(pszText,"RCD(%d,%d,%3.2f) = XYZ(%1.3f,%1.3f,%1.3f)",pointy,pointx,dDisparity,dX,dY,dZ);
            }
            else
            {
                sprintf(pszText,"Invalid");
    ...
    sprintf(pszText,"RC(%d,%d) = RGB(%d,%d,%d)",pointy,pointx,ucRed,ucGreen,ucBlue);

    pFrame->updateStatusBar(CPGRMainFrame::RGB,pszText);

	// Set flag = 1 if redframe's co-ordinates and X, Y points of mouse click are different.
	if (point.x <= m_ptStart.x + 15 || point.y <= m_ptStart.y + 15 ||
		point.x >= m_ptEnd.x + 15   || point.y >= m_ptEnd.y + 15    )
    {
        flag = 1;
    }   	
}
