...
CPGRStereoDoc::OnButtonStereoParams() 
{
   ...
   m_bStereoDialogActive = !m_bStereoDialogActive;
	...
	/*
 =======================================================================================================================
    Set Region of Interest Points
 =======================================================================================================================
 */
void CPGRStereoDoc::setROI(CPoint leftupperpt, CPoint rightlowerpt)
{
    start.x = leftupperpt.x;
    start.y = leftupperpt.y;
    end.x = rightlowerpt.x;
    end.y = rightlowerpt.y;
}
/*
 =======================================================================================================================
    Set Center Pixel (row,col) of ROI 
 =======================================================================================================================
 */
void CPGRStereoDoc::setCenterPixelXY(double Xrow, double Ycol)
{
    CenterPixX = Xrow;
    CenterPixY = Ycol;
}
...
/*
 =======================================================================================================================
   Export the Mill File having Resolution 81*97 i.e. 20"*24" seat
 =======================================================================================================================
 */
//Yixin Geng
void CPGRStereoDoc::OnFileExportHighResolution(float baseline, float smsurface)
{
	...
	//Yixin Geng
	//Split the point list into two parts: the upper one and lower one
	

	/*Converting Center Pixel (row,col) value to Real Camera Co-ordinates
     *Creating new C3dColourPointRC 
    */
    C3dColourPointRC    *CenterPixel = new C3dColourPointRC();
    /*
     *Initializing its XYZ values and (row,col) of ROI's center pixel values 
     */ 
	CenterPixel -> setXYZ(0,0,0);
    CenterPixel -> setColRow(CenterPixX,CenterPixY);

    C3dColourPointRC    *New = pointList->getNearestPosition(CenterPixel,pointList);
	
    double              realX = New->x();
    double              realY = New->y();

    /*
     * Converting ROI pixel points to real points ;
     * converting start point
     */
    C3dColourPointRC    *StartPixel = new C3dColourPointRC();
    StartPixel->setXYZ(0,0,0);
    StartPixel->setColRow((double)start.x,(double)start.y);

    
	C3dColourPointRC    *st = pointList->getNearestPosition(StartPixel,pointList);
    ...
    startr.y = sty;
     ...
     /*converting end pixel point to real points */
    C3dColourPointRC    *EndPixel = new C3dColourPointRC();
    EndPixel->setXYZ(0,0,0);
	
    EndPixel->setColRow((double)end.x,(double)end.y);
	
    C3dColourPointRC    *ed = pointList->getNearestPosition(EndPixel,pointList);
    ...
    Grid        *grid = new Grid(97,81,24.25,20.25);

    CPointList  *gridPoints = grid->getResolutionGrid(realX,realY);

	//Yixin Geng
	double centerLineLeftEnd_x = stx;
	double centerLineLeftEnd_y = (sty + edy)/2;
	double centerLineRightEnd_x = edx;
	double centerLineRightEnd_y = (sty + edy)/2;

	char        pszDefaultPointCloudFilename[] = "highresolutionPoints";
	//char        pszDefaultGridPointFilename[] = "grid.dat";
	
	//Yixin Geng
	//Calculating the exporting format file
		CPointList  *resolutionPoints_backPart = pointList->getResolutionToPointList
        (
            gridPoints,
			pointList->getRealWorldPoint(baseline),
            stx,
            sty,
            centerLineRightEnd_x,
            centerLineRightEnd_y,
            smsurface
        );

		CPointList  *resolutionPoints_seatPart = pointList->getResolutionToPointList
        (
            gridPoints,
			pointList->getRealWorldPoint(baseline),
            centerLineLeftEnd_x,
    ...
    if(fileDialog.DoModal() == IDOK)
    {
        //needed when we want to see the cloud point image
		//resolutionPoints->save(pszDefaultGridPointFilename,2);
		//pointList->save("pointcloud.pts",1);

        if(!resolutionPoints_backPart->saveToMillFormat(fileDialog.GetPathName()+"_back.dat",97,81))
        {
            AfxMessageBox("Failed to save point cloud file ");
        }

		if(!resolutionPoints_seatPart->saveToMillFormat(fileDialog.GetPathName()+"_seat.dat",97,81))
        {
            AfxMessageBox("Failed to save point cloud file ");
        }
    }
    ...
    /*if(fileDialog_seatPart.DoModal() == IDOK)
    {
        resolutionPoints_seatPart->save(pszDefaultGridPointFilename,2);
		pointList->save("pointcloud.pts",1);

        if(!resolutionPoints_seatPart->saveToMillFormat(fileDialog_seatPart.GetPathName(),97,81))
        {
            AfxMessageBox("Failed to save point cloud file ");
        }
...
/*
    CPointList  *resolutionPoints = pointList->getResolutionToPointList
        (
            gridPoints,
			pointList->getRealWorldPoint(baseline),
            stx,
    ...
    char        pszDefaultPointCloudFilename[] = "highresolutionPoints.dat";
    char        pszDefaultGridPointFilename[] = "grid.dat";
    CFileDialog fileDialog(FALSE, "dat", pszDefaultPointCloudFilename, NULL, NULL, NULL);
    ...
    if(fileDialog.DoModal() == IDOK)
    {
        ...
        if(!resolutionPoints->saveToMillFormat(fileDialog.GetPathName(),97,81))
        {
            AfxMessageBox("Failed to save point cloud file ");
        }
    }
